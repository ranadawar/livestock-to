...
const AllProductsVerify = ({ navigation }) => {
  ...
  React.useEffect(()=>{
    ...
    const addProd = async () => {
      ...
      if (snapshot.exists()) {
        ...
        setQrLink("https://live-stock-pro.web.app/Medical-Store/"+extractedProducts.flat()[0].id+"/hash/"+value.uid)
  ...
  const handleLink = (id) => {
    setId("red");
    const newLink = `https://live-stock-pro.web.app/Medical-Store/${id}/hash/${uid}`;
    setQrLink(newLink);
  };
