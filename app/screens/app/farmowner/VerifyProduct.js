...
const AllProducts = ({ navigation }) => {
  ...
  React.useEffect(()=>{
    ...
    const addProd = async () => {
      ...
      if (snapshot.exists()) {
        ...
        setQrLink("https://live-stock-pro.web.app/Farm-Store/"+extractedProducts.flat()[0].id+"/hash/"+value.uid)
  ...
  const handleDelete = async (id) => {
    ...
    const removeDataS = arr.map((prod)=>{
         return prod.products.filter((item)=>{
            if (item.id === id){
              ("hhh")
              return false
            }
    ...
    await set(dataRef, removeDataS);


    const dataRef1 = ref(getDatabase(), `/users/Farm-Item`);
    const snapshot1 = await get(dataRef1);
    const val1 = snapshot1.val();   
    const arr1 = Object.values(val1)

    const removeData = arr1.filter((item)=>{
      if (item.id === id){
        ("hhh")
        return false
      }
  ...
  const handleLink = (id) => {
    setId("red");
    const newLink = `https://live-stock-pro.web.app/Farm-Store/${id}/hash/${uid}`;
    setQrLink(newLink);
  };
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <View style={{backgroundColor: "#fff", padding: 20, marginVertical: 10, borderRadius: 10, alignItems:'center'}}>
                ...
                <TouchableOpacity onPress={()=> handleDelete(item.id)} style={styles.buyNow}>
